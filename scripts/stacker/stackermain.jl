...
function write_results(file, keys)
...
function process(cfile, mins, maxs, wrapped, quants, labels, outdir; tmin=0, tmax=24.0, nbatch=1_000, nsteps=2_000_000)
    ...
    StatsBase.sample(l, prior, RAM(M0=Minit, n=nsteps, show_progress=true), p0)
    #write_results(joinpath(outdir, replace(basename(cfile), ".h5"=>"_ha.csv")), keys)
    #return nothing
end
